...
lazy_static! {
    static ref DTYPE_SIZE_MAP: HashMap<&'static str, usize> = {
        let mut m = HashMap::new();
        m.insert("np.uint8", 1);
        m.insert("np.int8", 1);
        m.insert("np.int16", 2);
        m.insert("np.int32", 4);
        m.insert("np.int64", 8);
        m.insert("np.float32", 4);
        m.insert("np.float64", 8);
        m
    };
}
...
fn process_jsonl_file(
    ...
    // info!("Writing training_buckets to numpy.memmap file for megatron-lm");
    // let memmap_file = bin_dir.join(format!("processed_{}.memmap", file_name));
    // let writer = Index::writer(memmap_file, DataType::Int32)?;
    // let sizes = training_buckets_with_bos_eos
    //     .iter()
    //     .map(|bucket| bucket.len())
    //     .collect::<Vec<_>>();
    // let doc_idx = (0..training_buckets_with_bos_eos.len() as u64).collect::<Vec<_>>();
    // writer.write(&sizes, &doc_idx)?;

    info!("Writing training_buckets to numpy.memmap file");
    let memmap_file = bin_dir.join(format!("processed_{}.memmap", file_name));
    let total_size = HDR_SIZE
        + (training_buckets_with_bos_eos.len() * context_length * std::mem::size_of::<u32>());
