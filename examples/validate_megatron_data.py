...
def read_and_validate_mmap_file(index_path, data_path, doc_id=None, skip_warmup=False):
    ...
    index_buffer_mmap = np.memmap(index_path, mode='r', order='C')
    index_buffer = memoryview(index_buffer_mmap)
    ...
    pointers = np.frombuffer(index_buffer, dtype=np.int64, count=len_data,
                             offset=offset + sizes.nbytes)
    print(f"pointers: {pointers} and its shape is {pointers.shape}")

    print("    reading document index...")
    doc_idx = np.frombuffer(index_buffer, dtype=np.int64, count=doc_count,
    ...
    data_buffer_mmap = np.memmap(data_path, mode='r', order='C')
    data_buffer = memoryview(data_buffer_mmap)
    ...
    for doc_id in range(len(doc_idx)):
        ...
        for (pointer, size) in zip(pointers, sizes):
            ...
            np_array = np.frombuffer(data_buffer, dtype=dtype, count=size, offset=current_offset)
...
if __name__ == "__main__":
    print(os.getcwd())
    index_path = "data/output_folder/bin/processed_processed_sample_10.idx" # Replace with your actual file path
    data_path = "data/output_folder/bin/processed_processed_sample_10.bin" # Replace with your actual file path
